...
export const useProductImportStore = defineStore('productImport', () => {
    ...
    const importProducts = async (file) => {
        try {
            ...
            // Extraer información del resultado (campos confirmados del backend)
            const totalProcessed = responseData.total_processed || 0;
            const successfulImports = responseData.successful_count || 0;
            const failedImports = responseData.error_count || 0;
            const duplicatesSkipped = responseData.skipped_count || 0;
