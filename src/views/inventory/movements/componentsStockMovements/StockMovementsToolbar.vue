        ...
        <div class="toolbar-header">
            ...
            <div v-if="hasActiveFilters" class="active-filters-compact">
                <div class="filter-tags-compact">
                    ...
                    <span v-if="localDateFromFilter" class="filter-tag-compact">
                        <i class="pi pi-calendar"></i>
                        Desde: {{ formatDate(localDateFromFilter) }}
                        <button @click="clearDateFromFilter" class="tag-remove-compact">
                            <i class="pi pi-times"></i>
                        </button>
                    </span>

                    <span v-if="localDateToFilter" class="filter-tag-compact">
                        <i class="pi pi-calendar"></i>
                        Hasta: {{ formatDate(localDateToFilter) }}
                        <button @click="clearDateToFilter" class="tag-remove-compact">
                            <i class="pi pi-times"></i>
                        </button>
                    </span>
                </div>
...
.filter-date-compact {
    @apply bg-white/90 backdrop-blur-sm border-2 border-white/50 text-gray-800 rounded-lg transition-all duration-300;
    box-shadow: 0 4px 15px rgba(0, 0, 0, 0.1);
    min-width: 140px;
}

:deep(.filter-date-compact .p-inputtext) {
    @apply bg-transparent border-none text-gray-800 font-medium px-3 py-2;
}

:deep(.filter-date-compact .p-datepicker-trigger) {
    @apply text-gray-600;
}

/* Botón limpiar filtros compacto */
.clear-filters-btn-compact {
    @apply bg-white/90 backdrop-blur-sm border-2 border-white/50 text-gray-800 hover:bg-white hover:border-white/70 rounded-lg px-3 py-2 transition-all duration-300;
    box-shadow: 0 4px 15px rgba(0, 0, 0, 0.1);
    min-width: 40px;
    min-height: 40px;
}
...
.refresh-btn-compact {
    @apply bg-white/90 backdrop-blur-sm border-2 border-white/50 text-gray-800 hover:bg-white hover:border-white/70 rounded-lg transition-all duration-300;
    box-shadow: 0 4px 15px rgba(0, 0, 0, 0.1);
    min-width: 40px;
    min-height: 40px;
}
