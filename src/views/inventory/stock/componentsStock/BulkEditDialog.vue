...
const saveBulkUpdate = async () => {
    ...
    if (Object.keys(payload).length === 1) { // Solo scope
        toast.add({
            severity: 'warn',
            summary: 'Advertencia',
...
<template>
    <Dialog
        :visible="visible"
        modal
        header="Actualización Masiva de Stocks"
        :style="{ width: '70rem' }"
    ...
    >
        <div v-if="productData" class="space-y-6">
            ...
            <div v-if="previewData" class="bg-blue-50 p-4 rounded-lg">
                <h4 class="text-md font-semibold mb-3 text-blue-800">
                    Vista Previa - {{ previewData.total_stocks }} registros serán afectados
                </h4>
                
                <div class="max-h-40 overflow-y-auto">
                    <div
                        v-for="stock in previewData.stocks"
                        :key="stock.stock_id"
                        class="bg-white p-3 mb-2 rounded border text-sm"
                    >
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                <strong>{{ stock.warehouse_name }}</strong>
                                <span v-if="stock.batch_code" class="ml-2 text-gray-600">
                                    - Lote: {{ stock.batch_code }}
                                </span>
                            </div>
                            <div class="text-right text-xs text-gray-600">
                                ...
                                <div>
                                    Min: {{ stock.current_values.min_stock || 'N/A' }} | 
                                    Max: {{ stock.current_values.max_stock || 'N/A' }}
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <InputNumber
                            ...
                            :min="0"
                            :max-fraction-digits="2"
                            placeholder="Nuevo costo unitario"
                            class="w-full"
                        />
                    </div>
                </div>
            ...
            <div class="bg-yellow-50 border border-yellow-200 p-4 rounded-lg">
                <div class="flex items-start">
                    ...
                    <div class="text-sm text-yellow-800">
                        <strong>Advertencia:</strong> Esta acción modificará múltiples registros de stock. 
                        Los campos que no especifique mantendrán sus valores actuales.
                        Verifique la vista previa antes de continuar.
                    </div>
                </div>
            </div>
        </div>
        ...
        <template #footer>
            ...
            <Button
                ...
                icon="pi pi-check"
                :loading="loading"
                :disabled="!previewData || previewData.total_stocks === 0"
