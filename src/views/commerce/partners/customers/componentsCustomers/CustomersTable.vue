...
const { customers, loading } = defineProps({
...
const initFilters = () => ({
    ...
    identity_document: { operator: FilterOperator.AND, constraints: [{ value: null, matchMode: FilterMatchMode.STARTS_WITH }] },
...
<template>
    ...
    >
            ...
            <div class="table-header">
                ...
                <div class="header-content">
                    ...
                    <div class="actions-section">
                        <Button type="button" icon="pi pi-file-excel" label="Exportar" class="export-btn" @click="exportCustomers()" v-tooltip.top="'Exportar clientes a Excel'" :disabled="!customers.length" />
                    </div>
                </div>
            </div>
        </template>
        ...
        <Column field="identity_document" header="Documento" sortable style="min-width: 6rem; max-width: 8rem">
            <template #body="{ data }">
                <div class="dni-badge">
        ...
        <Column field="identity_document_type" header="Tipo Doc." sortable style="min-width: 6rem; max-width: 8rem">
            <template #body="{ data }">
                <div class="position-tag">
                    <i class="pi pi-id-card"></i>
                    <span>{{ data.identity_document_type?.toUpperCase() || '-' }}</span>
                </div>
            </template>
        </Column>
        ...
        <Column field="is_active" header="Estado" sortable style="min-width: 4rem; max-width: 6rem">
            <template #body="{ data }">
                <div class="flex justify-center">
                    <div v-if="data.is_active === true" class="status-badge active">
                        <i class="pi pi-check"></i>
                        <span>Activo</span>
                    </div>
                    <div v-else-if="data.is_active === false" class="status-badge inactive">
...
.status-badge.pending {
    @apply bg-gray-100 dark:bg-gray-900/30 text-gray-800 dark:text-gray-300 border border-gray-200 dark:border-gray-700;
}
