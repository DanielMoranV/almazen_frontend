...
<template>
                ...
                <div v-else class="space-y-4">
                    <div v-for="(item, index) in cart" :key="index"
                        ...
                        <div class="flex items-start justify-between mb-3">
                            ...
                            <Button @click="$emit('remove-from-cart', index)" icon="pi pi-trash" size="small"
                        ...
                        <div class="flex items-center justify-between">
                            <div class="flex items-center space-x-2 bg-white rounded-lg p-1 border">
                                <Button @click="$emit('update-quantity', item, item.quantity - 1)" icon="pi pi-minus"
                                    size="small" text rounded class="w-8 h-8 hover:bg-gray-100" />
                                <span class="w-8 text-center font-bold text-lg">{{ item.quantity }}</span>
                                <Button @click="$emit('update-quantity', item, item.quantity + 1)" icon="pi pi-plus"
                                    size="small" text rounded class="w-8 h-8 hover:bg-gray-100" />
                            </div>
                    ...
                    <div class="space-y-3 pt-4">
                        <Button @click="$emit('open-multiple-payment-dialog')" label="Procesar Pago"
                            :disabled="!canOperateWithoutSession" icon="pi pi-credit-card"
                            class="w-full h-14 text-lg font-bold" size="large" severity="success"
                            :loading="loading" />
                        <Button @click="$emit('clear-cart')" label="Limpiar Carrito" icon="pi pi-trash"
    ...
    <!-- Mobile Cart Dialog -->
    <Dialog :visible="showCartSummary" @update:visible="$emit('update:showCartSummary', $event)" header="Carrito de Compras" :modal="true" class="lg:hidden"
        ...
        <div v-else class="space-y-4">
            <div v-for="(item, index) in cart" :key="index"
                ...
                <div class="flex items-start justify-between mb-3">
                    ...
                    <Button @click="$emit('remove-from-cart', index)" icon="pi pi-trash" size="small" severity="danger" text
                        rounded class="ml-2" />
                </div>

                <div class="flex items-center justify-between">
                    <div class="flex items-center space-x-2 bg-white rounded-lg p-1 border">
                        <Button @click="$emit('update-quantity', item, item.quantity - 1)" icon="pi pi-minus" size="small"
                            text rounded class="w-10 h-10" />
                        <span class="w-8 text-center font-bold text-lg">{{ item.quantity }}</span>
                        <Button @click="$emit('update-quantity', item, item.quantity + 1)" icon="pi pi-plus" size="small" text
            ...
            <div class="space-y-3 pt-4">
                <Button @click="$emit('open-multiple-payment-dialog'); $emit('update:showCartSummary', false)" label="Procesar Pago"
                    :disabled="!canOperateWithoutSession" icon="pi pi-credit-card"
                    class="w-full h-14 text-lg font-bold" size="large" severity="success" :loading="loading" />
                <Button @click="$emit('clear-cart')" label="Limpiar Carrito" icon="pi pi-trash" severity="secondary" outlined
                    class="w-full h-12" />
            </div>
        </div>
    </Dialog>
