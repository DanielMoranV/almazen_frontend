<template>
    <div class="product-sale-search">
                ...
                <div class="title-section">
                    ...
                    <div>
                        ...
                        <p class="text-sm text-gray-600 dark:text-gray-300">
                            Busque por código de barras, SKU o nombre del producto
                        </p>
                    </div>
                </div>
            </div>
        ...
        <div class="search-form">
            <div class="search-inputs">
                ...
                <div class="warehouse-field" v-if="showWarehouseFilter">
                    <label for="warehouse-select" class="search-label">
                        Filtrar por almacén
                    </label>
                    <Dropdown
                        id="warehouse-select"
        ...
        <div v-if="getSaleSearchMessage" class="search-message">
            <Message
                :severity="saleProductsList.length > 0 ? 'success' : 'info'"
                :closable="false"
            >
        ...
        <div v-if="!isLoadingSaleProducts && saleProductsList.length > 0" class="search-results">
            <div class="results-header">
                <h3 class="results-title">
                    Productos Encontrados ({{ totalSaleProducts }})
                </h3>
                ...
                <Column field="name" header="Producto" :sortable="true">
                    <template #body="{ data }">
                        <div class="product-info">
                            ...
                            <Badge
                                v-if="hasExpiringBatches(data.id)"
                ...
                <Column header="Stock Disponible" :sortable="false">
                    <template #body="{ data }">
                        <div class="stock-info">
                            <div
                                v-for="warehouse in data.available_stock"
                            ...
                            >
                                <div class="warehouse-header">
                                    ...
                                    <Badge
                                        :value="warehouse.total_stock + ' unidades'"
                                        :severity="getStockSeverity(warehouse.total_stock)"
                                    />
                                </div>
                                ...
                                <div v-if="data.requires_batches && warehouse.batches" class="batches-info">
                                    <div
                                        v-for="batch in warehouse.batches.slice(0, 3)"
                                        :key="batch.batch_id"
                                        class="batch-item"
                                    >
                                        ...
                                        <span
                                            v-if="batch.expiration_date"
                                            :class="getBatchExpirationClass(batch)"
                                        >
                                            {{ formatDate(batch.expiration_date) }}
                                        </span>
                                        <span class="batch-price">${{ batch.sale_price }}</span>
                                    </div>
                                    <small v-if="warehouse.batches.length > 3" class="more-batches">
                                        +{{ warehouse.batches.length - 3 }} lotes más
                                    </small>
                                </div>
                ...
                <Column header="Acciones" :exportable="false">
                    <template #body="{ data }">
                        <div class="action-buttons">
                            <Button
                                icon="pi pi-plus"
                                v-tooltip.top="'Agregar a venta'"
                                @click="$emit('add-to-sale', data)"
                                size="small"
                                class="add-btn"
                            />
                            <Button
            ...
            <div v-else class="products-cards">
                ...
                >
                    <div class="card-header">
                        ...
                        <Badge
                            v-if="hasExpiringBatches(product.id)"
                            value="Próximo a vencer"
                            severity="warning"
                        />
                    </div>
                    ...
                    <div class="card-stock">
                        <div
                            v-for="warehouse in product.available_stock"
                            :key="warehouse.warehouse_id"
                            class="warehouse-card-stock"
                        >
                            ...
                            <div v-if="product.requires_batches && warehouse.batches" class="batch-summary">
                                <div class="batch-info">
                                    ...
                                    <span class="price-range">
                                        ...
                                        <span v-if="warehouse.batches.length > 1">
                                            - ${{ Math.max(...warehouse.batches.map(b => b.sale_price)) }}
                                        </span>
                                    </span>
                                </div>
                            </div>
                    ...
                    <div class="card-actions">
                        <Button
                            ...
                            icon="pi pi-plus"
                            @click="$emit('add-to-sale', product)"
            ...
            <Paginator
                v-if="viewMode === 'cards'"
                :first="cardsPaginatorFirst"
                :rows="cardsPaginatorRows"
                :totalRecords="totalSaleProducts"
                @page="onCardsPageChange"
        ...
        <!-- Estado vacío -->
        <div
            v-if="!isLoadingSaleProducts && !getSaleSearchMessage && saleProductsList.length === 0 && localSearchTerm"
            class="empty-state"
        >
...
const onSearchInput = () => {
    if (props.autoSearch) {
        productsStore.searchProductsForSaleWithDebounce(
