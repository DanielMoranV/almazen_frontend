...
const hasOverdueCredits = computed(() => {
    return props.customer.has_overdue_credits || 
           props.credits.some(credit => credit.is_overdue);
});

// Contar créditos vencidos
const overdueCount = computed(() => {
    return props.customer.overdue_credits_count || 
           props.credits.filter(credit => credit.is_overdue).length || 0;
});
...
const maxOverdueDays = computed(() => {
    ...
    return Math.max(...overdueCredits.map(credit => {
...
<template>
    <div v-if="hasOverdueCredits || !compact" class="overdue-credit-alert">
        ...
        <InlineMessage 
            ...
            class="alert-message w-full"
            :class="{ 'compact-message': compact }"
        >
            <template #icon>
                <i :class="alertIcon"></i>
            </template>
            ...
            <div class="alert-content">
                ...
                <div v-if="showActions && hasOverdueCredits" class="alert-actions">
                    <Button
                        icon="pi pi-dollar"
                        :label="compact ? '' : 'Registrar Pago'"
                        size="small"
                        class="p-button-sm"
                        :class="alertSeverity === 'error' ? 'p-button-danger' : 'p-button-warning'"
        ...
        <div v-if="!compact && hasOverdueCredits && credits.length > 0" class="overdue-details">
            ...
            <div class="overdue-list">
                <div 
                    v-for="credit in credits.filter(c => c.is_overdue)" 
                    :key="credit.id"
                    class="overdue-item"
                >
                    <div class="item-info">
                        ...
                        <div class="due-date">
                            Venció el {{ formatDate(credit.due_date) }}
                            ({{ getDaysOverdue(credit) }} días)
                        </div>
                    </div>
